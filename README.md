...
---
Friendly Reminder：
HA的插件账号一定要是一个单独的账号，如果有任何手机登录相同账号，会导致HA插件端口时不时被踢下线，燃烧状态轮询和按键状态都会有随机的掉线或不可用，研究了半天才发现。
...
- **热水器控制**: 设置生活热水温度。
- **一键循环开关**: 解决一键循环在HA中的支持，后续可链接实体按钮。
...
- **"No configuration found" (未找到配置)**: 请确认您的设备型号是否受支持。
- 目前主要支持 G56和Q85 系列。(其他设备待后续计划添加)
- **实体不可用**: 请检查设备在林内 App 中是否在线。本集成依赖于云端 API。
## 贡献与支持

感谢palafin02back的原创插件，本项目是如下项目修改而来
https://github.com/palafin02back/rinnai_integration
